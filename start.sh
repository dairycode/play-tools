...
if lsof -i :8081 > /dev/null 2>&1; then
    echo -e "${YELLOW}警告: 后端服务已在运行 (端口 8081)${NC}"
else
    # 启动后端服务
    ...
    echo -e "${GREEN}✓ 后端服务已启动 (PID: $BACKEND_PID, 端口: 8081)${NC}"
    echo -e "${GREEN}  日志文件: logs/backend.log${NC}\n"
fi

# 检查前端进程是否已在运行
if lsof -i :5173 > /dev/null 2>&1; then
    echo -e "${YELLOW}警告: 前端服务已在运行 (端口 5173)${NC}"
else
    # 启动前端服务
    ...
    echo -e "${GREEN}✓ 前端服务已启动 (PID: $FRONTEND_PID, 端口: 5173)${NC}"
